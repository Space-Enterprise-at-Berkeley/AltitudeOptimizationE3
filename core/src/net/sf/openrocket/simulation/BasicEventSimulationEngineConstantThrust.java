...
//Special case of basic evens simulation engine dispersion analysis:
//BIG differences: 1) stops once apogee is reached

public class BasicEventSimulationEngineConstantThrust implements SimulationEngine {
	
	private static final Translator trans = Application.getTranslator();
	private static final Logger log = LoggerFactory.getLogger(BasicEventSimulationEngineConstantThrust.class);
	...
	private double windSpeedAvg = 0;
	private double windDirectionAvg = 0;
	...
	public double getAverageWindSpeed(){return windSpeedAvg/fullFlightStatus.size();}
	public double getAverageWindDirection(){return windDirectionAvg/fullFlightStatus.size();}



	//updates the exit pressure of the motor
	public void setMotorPressure(){
		double altitude = currentStatus.getRocketWorldPosition().getAltitude();
		double curr_pressure = currentStatus.getSimulationConditions().getAtmosphericModel().getConditions(altitude).getPressure();
		((ThrustCurveMotor)((ArrayList<MotorClusterState>) currentStatus.getActiveMotors()).get(0).getMotor()).setAmbientPressure(curr_pressure);
	}
...
//
		...
		//updates average wind and direction for later table export
		windSpeedAvg += wind;
...
//				System.out.println(currentStatus.getSimulationTime() + " " + result);
				...
				setMotorPressure(); //updates the current pressure in the motor
...
//				System.out.println("simulation time is " + currentStatus.getSimulationTime());
				...
				if (SimulationListenerHelper.firePreStep(currentStatus)) {
					...
					//TODO NEW ERIC CODE
					//Doesn't properly output wind updates (pointer issue)
...
//
	...
	private FlightData computeCoastTime() throws SimulationException {
		try {
			...
			BasicEventSimulationEngineConstantThrust e = new BasicEventSimulationEngineConstantThrust();

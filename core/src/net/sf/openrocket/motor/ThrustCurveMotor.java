...
public class ThrustCurveMotor implements Motor, Comparable<ThrustCurveMotor>, Serializable {
	...
	//TODO: New Eric Variable
	private boolean activePressureThrust = false;
	private double exitPressure = 0.6*101325; // in pa
	private double exitArea = 0.01246898124; // in m^2

	private double curr_pressure = 101315; //current pressure, in pa
	...
	public double getThrust( final double motorTime ){
		...
		double totalThrust;
		if (activePressureThrust){
			totalThrust = pressureThrust() + thrustAtTime;
		} else{
			totalThrust = thrustAtTime;
		}
//		System.out.println(totalThrust);
		return totalThrust;
	...
	//TODO: NEW ERIC CODE:
	/*
	Incorporates exit pressure thrust
	 */

	//runs once per sim, allows iteration over exit pressures
	public void setExitPressure(double exitPressure){
		this.exitPressure = exitPressure*101325; //converts from atm to PA
		activePressureThrust = true;
	}

	//Runs every time sep
	public void setAmbientPressure(double curr_pressure){
		this.curr_pressure = curr_pressure;
	}



	public double pressureThrust(){
		return exitArea*(exitPressure - curr_pressure);
	}
